  ...
  var qry = JSON.parse(filter);
    ...
    function(callback){
      ...
      async.each(pupils, function(pupil, callback){
        ...
        var col = db.collection('scripts');
        col.insert({booklet:booklet._id, owners:task.owners, task:task._id, lastName: pupil.lastName, firstName: pupil.firstName, school: pupil.school, class: pupil.class, tier: pupil.tier, pupil:pupil._id ,label:false, processed:false, createdAt:moment().format()}, function(err, result){
          if (err) return callback(err);
          callback();
        });
      }, function(err){
