...
genQrCode = function(db, task, script, n, callback){
  var qrcode = _.sample(['A','B','C','D','E','F','G','H','I','J','K','L','M','N','P','Q','R','S','T','U','V','W','X','Y','Z'],n).join('');
  ...
  col.find({task:task._id, qrcode:qrcode},{},{limit:1}).count(function(err, count){
    ...
    if(count>0){
      ...
      genQrCode(db, task, script, n, callback);
    } else {
      col.update({_id:script._id}, {$set:{qrcode:qrcode}}, function(err, result){
        if(err) callback(err);
        callback (err,result);
      });
    }
  ...
  var qry = JSON.parse(filter);
    ...
    function(callback){
      ...
      async.each(pupils, function(pupil, callback){
        ...
        var col = db.collection('scripts');
        col.insert({booklet:booklet, owners:task.owners, task:task._id, pupil:pupil._id ,processed:false}, function(err, result){
          if (err) return callback(err);
          callback();
        });
      }, function(err){
    ...
    function(callback){
      //Find scripts that need a qrcode
      var col = db.collection('scripts');
      col.find({task:task._id, qrcode:{$exists:false}}).toArray(function(err, items) {
        if (err) return callback(err);
        if (items.length === 0){
          return callback(new Error('No qr codes to update'));
        }
        scripts = items;
        callback();
      });
    ...
    function(callback){
      ...
      async.eachSeries(scripts, function(script, callback){
        genQrCode(db, task, script ,5, function(err, qrcode){
          if(err) return callback(err);
          callback();
        });
      }, function(err){
